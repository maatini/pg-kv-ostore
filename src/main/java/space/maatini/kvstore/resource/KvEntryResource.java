...
public class KvEntryResource {
        ...
        @PermitAll
        public Uni<KvEntryDto.Response> get(
                        @Parameter(description = "Bucket name") @PathParam("bucket") String bucket,
                        @Parameter(description = "Key name") @PathParam("key") String key) {
                return kvService.get(bucket, key).map(entry -> KvEntryDto.Response.from(entry, bucket));
        }
        ...
        @PermitAll
        public Uni<KvEntryDto.Response> getRevision(
                        @Parameter(description = "Bucket name") @PathParam("bucket") String bucket,
                        @Parameter(description = "Key name") @PathParam("key") String key,
                        @Parameter(description = "Revision number") @PathParam("revision") Long revision) {
                return kvService.getRevision(bucket, key, revision)
        ...
        @PermitAll
        public Uni<List<KvEntryDto.Response>> getHistory(
                        @Parameter(description = "Bucket name") @PathParam("bucket") String bucket,
                        @Parameter(description = "Key name") @PathParam("key") String key,
                        @Parameter(description = "Maximum number of revisions") @QueryParam("limit") @DefaultValue("10") int limit) {
                return kvService.getHistory(bucket, key, limit).map(list -> list.stream()
        ...
        @RolesAllowed({ "admin", "kv-write" })
        public Uni<KvEntryDto.Response> delete(
                        @Parameter(description = "Bucket name") @PathParam("bucket") String bucket,
                        @Parameter(description = "Key name") @PathParam("key") String key) {
                return kvService.delete(bucket, key).map(entry -> KvEntryDto.Response.from(entry, bucket));
        }
        ...
        @RolesAllowed({ "admin" })
        public Uni<ApiResponse.Count> purge(
                        @Parameter(description = "Bucket name") @PathParam("bucket") String bucket,
                        @Parameter(description = "Key name") @PathParam("key") String key) {
                return kvService.purge(bucket, key).map(ApiResponse.Count::new);
        }
}
